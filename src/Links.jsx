...
const lilypadButtonClassName = `
  ...
  rounded-3xl bg-[#4f7908] px-8 py-5 text-lg font-semibold text-[#1A3706]
...
function LilypadButton({ children, href, onClick }) {
  ...
  const content = (
    <>
      <span
        aria-hidden="true"
        className="absolute inset-0 bg-gradient-to-b from-[#638b0f] via-[#4f7908] to-[#315706]"
      />
  ...
  if (onClick) {
    return (
      <button type="button" onClick={onClick} className={lilypadButtonClassName}>
        {content}
      </button>
    );
  }
  ...
  return (
    <a
      ...
      className={lilypadButtonClassName}
    >
      {content}
    </a>
  );
